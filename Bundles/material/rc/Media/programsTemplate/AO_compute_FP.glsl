...
//    vec2 dc_y = -vec2(c.x,c.y-1) + vec2(c.x,c.y+1); // dérivée du point c en y
//    vec2 dc_x = -vec2(c.x-1,c.y) + vec2(c.x+1,c.y);

//    vec2 nc = normalize(cross(dc_y,dc_x));
...
//         int m = clamp (findMSB(int (h)) - log_q);
        ...
        vec2 pixel_i = vec2(vec2(gl_FragCoord) + vec2(h_i*u_i)) /pow(2.0,m_i);
        ...
        vec2 q_ = z_i * vec2( ((1.0f - MVP[0][2])/MVP[0][0]) - ((2.0f*(pixel_i.x+0.5f)) / (viewport_w_i* MVP[0][0])) ,
                            ((1.0f + MVP[1][2])/MVP[1][1]) - ((-2.0f*(pixel_i.y+0.5f)) / (viewport_h_i* MVP[1][1])) );
...
//        sum += (max(0.0f,(dot (vi,nc) + zc* beta)/(dot(vi,vi)+epsilon)));
        ...
        float f = max (pow(r,2.0) - dot(vi,vi),0.0);
...
//    float a = pow (max (0.0, 1.0f-(2.0f* sigma *sum)/s),k);
    float a = max (0.0,1.0 - sum * pow(r,6)*(5.0 / s));
    ...
    float tmp = floor(zc * 256.0);

    float cszToKey = clamp (zc * (1.0/FAR_PLANE),0.0,1.0);

    fragColor.rgb = vec3(a ,tmp*(1.0/256.0),cszToKey*256.0 - tmp);
